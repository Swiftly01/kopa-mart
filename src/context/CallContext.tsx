...
export function CallProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const onPeerJoined = () => {
      console.log("[call-debug] onPeerJoined fired, pcRef.current exists:", !!pcRef.current);
      if (!pcRef.current) {
    ...
    const onOffer = async (data: { sdpType: "offer" | "answer"; sdp: string }) => {
      console.log("[call-debug] onOffer received, pcRef.current exists:", !!pcRef.current);
  ...
  function createPeerConnection(callId: string): RTCPeerConnection {
    ...
    console.log("[call-debug] iceServers in use:", iceServersRef.current);
    ...
    pc.onicecandidate = (event) => {
      ...
      // eslint-disable-next-line no-console
      console.log("[call-debug] local ICE candidate:", event.candidate.type, event.candidate.candidate);
    ...
    pc.ontrack = (event) => {
      // eslint-disable-next-line no-console
      console.log("[call-debug] ontrack fired, remote stream:", event.streams[0]?.id);
  ...
  async function createOfferIfCaller() {
    ...
    const pc = pcRef.current;
    if (!pc) {
      console.warn("[call-debug] createOfferIfCaller: no pc yet, bailing");
      return;
    }
    try {
      console.log("[call-debug] creating offer...");
      const offer = await pc.createOffer();
      await pc.setLocalDescription(offer);
      console.log("[call-debug] offer created + set, emitting call_offer");
